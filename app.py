...
# --- UI "1 clic" (à enlever après migration) ---
if st.sidebar.checkbox("MIGRATION (ONE SHOT)", value=False):
    st.sidebar.warning("⚠️ À utiliser une seule fois, puis à supprimer du code.")
    if st.sidebar.button("🚚 Migrer JSON → DB"):
        if engine is None:
            st.sidebar.error("DB non connectée")
        else:
            try:
                counts = migrate_json_to_db(engine)
                st.sidebar.success("✅ Migration terminée !")
                st.sidebar.write(counts)
            except Exception as e:
                st.sidebar.error(f"❌ Migration KO: {e}")
